...
saved_train_path = f'{os.getcwd()}/.data/wmt14/saved_train.pickleno'
saved_valid_path = f'{os.getcwd()}/.data/wmt14/saved_valid.pickleno'
saved_test_path = f'{os.getcwd()}/.data/wmt14/saved_test.pickleno'

saved_train_ds_path = f'{os.getcwd()}/saved_train_ds_path.pickleno'
saved_valid_ds_path = f'{os.getcwd()}/saved_valid_ds_path.pickleno'
saved_max_len_sentence_path = f'{os.getcwd()}/saved_max_len_sentence.pickleno'
...
#'''
#can not be used,,, source sentence letters seem to be broken,,,
#'''
# del train.examples[4391595:4391605]
...
if share_vocab:
    ...
    sys.stdout.flush()
    TRG_cpy = copy.deepcopy(TRG.vocab)
    ...
    TRG.vocab = TRG_cpy
    print('Get merged source vocabulary size: ', len(SRC.vocab))
    print('Recover target vocab size: ', len(TRG.vocab))
    sys.stdout.flush()
...
save_total_num = 10
nstep_check = CheckpointEveryNSteps(save_batch_frequency=hparams.n_steps//save_total_num)

# max_steps = int(math.floor((len(train.examples) * hparams.n_epochs) / args.gpus))
# max_steps = int(math.floor(hparams.batch_size * accumul_num * hparams.n_steps) / args.gpus)
# val_check_interval = int(math.ceil(len(train.examples)//(hparams.batch_size*args.gpus))/2)
accumul_num = 128
check_interval = 20
